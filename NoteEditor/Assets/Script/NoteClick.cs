...
public class NoteClick : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        ...
        if (CompareTag(SpeedNoteTag) || CompareTag(EffectNoteTag)) { _noteObject = gameObject; }
        else { _noteObject = this.transform.parent.parent.gameObject; }
        ...
        if (_noteObject.tag == NormalNoteTag || _noteObject.tag == BottomNoteTag)
        {
            ...
            _noteObject.GetComponent<NoteOption>().EnableCollider(false);
        }
